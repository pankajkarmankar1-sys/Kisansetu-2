  ...
  const { error } = await supabase.from("users").upsert(
    {
      ...
      farm_address: u.farmAddress || null,
      acres: parseFloat(u.acres || 0),
      aadhaar_done: !!u.aadDone,
      sat_done: !!u.satDone,
      role: "customer",
  ...
  const { error } = await supabase.from("drivers").upsert(
    {
      ...
      tractor_num: d.tractorNum || null,
      tractor_det: d.tractorDet || null,
      aadhaar_done: !!d.aadDone,
      lic_done: !!d.licDone,
      approved: false,
      updated_at: new Date().toISOString(),
    },
...
export async function sbGetDriver(phone) {
  const { data } = await supabase
    .from("drivers").select("*").eq("phone", phone).maybeSingle();
  return data;
}
  ...
  const { error } = await supabase.from("bookings").upsert(
    {
      ...
      service_name_hi: b.serviceNameHi || null,
      icon: b.icon || null,
      acres: parseFloat(b.acres || 0),
      amount: b.amount || 0,
      date: b.date || null,
      note: b.note || null,
      khet_name: b.khetName || null,
      khet_village: b.khetVillage || null,
      ...
      pay_method: b.payMethod || null,
      pay_ref: b.payRef || null,
...
export async function sbGetCustomerBookings(phone) {
  const { data } = await supabase
    .from("bookings")
    .select("*")
...
// ── Khets ──────────────────────────────────────────────────────────────
export async function sbSaveKhets(phone, khets) {
  if (!khets || !khets.length) return;
  const rows = khets.map((k) => ({
    id: k.id || "K" + Date.now() + Math.random().toString(36).slice(2,5),
    user_phone: phone,
    name: k.name,
    village: k.village || null,
  ...
  if (error) console.error("sbSaveKhets:", error.message);
}
